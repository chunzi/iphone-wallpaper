...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{IPHONEWALLPAPER_WEBAPP_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{IPHONEWALLPAPER_WEBAPP_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require iPhoneWallpaper::WebApp;

    iPhoneWallpaper::WebApp->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
iphonewallpaper_webapp_server.pl - Catalyst Testserver

=head1 SYNOPSIS

iphonewallpaper_webapp_server.pl [options]
